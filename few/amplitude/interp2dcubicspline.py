...
# Cython/C++ imports
from pyInterp2DAmplitude import pyAmplitudeGenerator, pyAmplitudeGenerator_Kerr, pyAmplitudeGenerator_Kerr_full 

# Python imports
from few.utils.baseclasses import SchwarzschildEccentric, AmplitudeBase, AmplitudeBaseKerrCircular, KerrCircular, AmplitudeBaseKerrCircular_full, KerrCircular_full
...
class Interp2DAmplitudeKerrCircular_full(AmplitudeBaseKerrCircular_full, KerrCircular_full):
    """Calculate Teukolsky amplitudes by 2D Cubic Spline interpolation.
    ...
    def __init__(self, **kwargs):
        ...
        KerrCircular_full.__init__(self, **kwargs)
        AmplitudeBaseKerrCircular_full.__init__(self, **kwargs)

        # check if you have the necessary file
        # it will download from Zenodo if the user does not have it.
        few_dir = dir_path + "/../../"

        fp = "Clm00_e0.0_lmax_30_full.h5"
        check_for_file_download(fp, few_dir)

        self.amplitude_generator_full = pyAmplitudeGenerator_Kerr_full(
            self.lmax, self.nmax, few_dir
        )

    def attributes_Interp2DAmplitudeKerrCircular_full(self):
        """
        attributes:
            amplitude_generator (obj): C++ class that performs the bicubic
                interpolation. It stores all of the splines during initialization
    ...
    def get_amplitudes_kerr_full(self, a, p, e, *args, specific_modes=None, **kwargs):
        """Calculate Teukolsky amplitudes for Schwarzschild eccentric.
        ...
        teuk_modes = self.amplitude_generator_full(
            a,
            p,
            e,
            l_arr.astype(np.int32),
