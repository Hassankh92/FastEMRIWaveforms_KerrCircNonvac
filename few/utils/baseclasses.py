...
class KerrCircular_full(ParallelModuleBase, ABC):
    ...
    def sanity_check_traj_full(self, a, p, e):
        """Sanity check on parameters output from thte trajectory module.
        ...
        if np.any(a > 0.99):
            raise ValueError("Members of a array are more than the provided input data.")
        if np.any(a < -0.99):
            raise ValueError("Members of a array are less than the provided input data.")
      

        if np.any(p < 0.0):
            raise ValueError("Members of p array are less than zero.")

    def sanity_check_init_full(self, M, mu, a0, p0, e0):
        """Sanity check initial parameters.
        ...
        if a0 < -0.99:
            raise ValueError(
                "Initial spin below -0.99 is outside of our waveform validity. (a0={})".format(a0)
            )
        if e0 != 0.0:
            raise ValueError(
                "This module is limitted to circular orbits only. (e0={})".format(e0)
        ...
        if p0 > 40.0:
            ...
            if a0 < 0.0:
                x = -1.0
            ps = get_separatrix(a0, 0.0, x)
            u0 = np.log(p0 - ps + 3.93)
            if u0 > 3.818:
                raise ValueError(
                    "Initial p0 is too large (p0={}). Must be u0 <= 3.8186394259".format(
                        u0
                    )
                )
...
class AmplitudeBaseKerrCircular_full(ABC):
    """Base class used for amplitude modules.
    ...
    @classmethod
    def get_amplitudes_kerr_full(self, *args, **kwargs):
        """Amplitude Generator
    ...
    def __call__(self, *args, specific_modes=None, **kwargs):
        ...
        return self.get_amplitudes_kerr_full(*args, specific_modes=specific_modes, **kwargs)
